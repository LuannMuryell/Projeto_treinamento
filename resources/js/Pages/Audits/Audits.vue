                    ...
                    <v-table>
                        ...
                        <tbody>
                            <tr v-for="audit in filteredAudits" :key="audit.id">
                                ...
                                <td class="py-2 px-4 text-center">
                                    {{ viewEvent(audit.event) }}
                                </td>
                                <td class="py-2 px-4 text-center">
                                    {{ formatDateTime(audit.created_at) }}
                                </td>
                                <td class="py-2 px-4 text-center">
                                    {{ viewTable(audit.auditable_type) }}
                                </td>
                                <td class="py-2 px-4 text-center">
                                    {{ audit.auditable_id }}
                                </td>
        ...
        <v-dialog v-model="dialog" max-width="600">
            <v-card>
                <v-card-title class="text-center text-h5 pa-4">
                    <v-icon left size="24" class="me-1" style="margin-top: -3px"
                ...
                <v-card-text>
                    <v-list lines="one" density="compact" readonly>
                        <v-list-item>
                            <v-list-item-title
                                ><strong>ID:</strong>
                                {{ selectedAudit.id }}</v-list-item-title
                            >
                        </v-list-item>
                        <v-list-item>
                            <v-list-item-title
                        ...
                        <v-list-item>
                            <v-list-item-title
                                ...
                                {{
                                    viewTable(selectedAudit.auditable_type)
                                }}</v-list-item-title
                            >
                        </v-list-item>
                        <v-list-item>
                        ...
                        <v-list-item>
                            <v-list-item-title
                                ><strong
                                    >Dados Anteriores:</strong
                                ></v-list-item-title
                            >
                            <v-list-item-subtitle
                                v-if="
                                    Object.keys(selectedAudit.old_values).length
                                "
                            >
                                <v-list>
                                    <v-list-item
                        ...
                        <v-list-item>
                            ...
                            <v-list-item-subtitle
                                v-if="
                                    Object.keys(selectedAudit.new_values).length
                                "
                            >
                                <v-list dense>
                                    <v-list-item
                        ...
                        <v-list-item>
                            <v-list-item-title
                                ><strong>URL:</strong>
                                {{ selectedAudit.url }}</v-list-item-title
                            >
                        </v-list-item>
                        <v-list-item>
                            <v-list-item-title
...
const filteredAudits = computed(() => {
    return props.audits.data.filter((audit) => {
        ...
        const userName = audit.user ? audit.user.name.toLowerCase() : "";
        const event = audit.event ? audit.event.toLowerCase() : "";
        const table = viewTable(audit.auditable_type).toLowerCase();
        const date = formatDateTime(audit.created_at);
